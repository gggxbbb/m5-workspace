...
> 状态：**已定稿，方向弃用（2026-08-16）**。真机验证完成：断开重连已修复（onRead 跨扇区读）、E: 盘稳定挂载；但 **Windows FAT 缓存使已挂载卷的内容更新不可见**（实测 `fails=0` 证明主机挂载后不再读盘），"轮询刷新"只在挂载瞬间呈现最新值。已实现 SCSI Unit Attention（ASC 0x28）介质变化通知 + 随机卷序列号以缓解，体验仍受重挂频率限制。**整体方向让位于 `tools/usb-hid`（USBHIDVendor 双向通道）**——无缓存、天然双向、实现更简单。代码保留供参考。
>
