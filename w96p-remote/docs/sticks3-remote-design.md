...
```
g0    = 进入时 accel 滑动均值（静止时加速度计读支撑力方向 = 体感"上"！）
L     = 设备长轴（朝顶端）在驱动系中的方向 = (-1,0,0)
down  = g0 / |g0|   （变量名叫 down，物理上是"上"，公式按"上"推导）
right = normalize(cross(L, down))      // 体感"右"——指向用户右手
fwd   = normalize(cross(right, down))  // 体感"前"——使上举 p>0（升速）
```

**真机标定（2026-08-03，imu-calib 三姿势实测）**：M5Unified 的 BMI270 驱动轴系 ≠ 官方 IMU 图，绕 Z 旋转 90°——驱动 +X = 图 -Y（朝 USB 端）、驱动 +Y = 图 +X（设备右侧）、驱动 +Z = 图 +Z。L 必须取 (-1,0,0)。
性质：设备倒置（旋转 180°）时 g0 翻转 → right 自动翻转，**方向语义跟着手走**。
退化情形：设备完全竖直握持（L ∥ down）时 cross 无定义 → 沿用上一次的基；首次使用则屏幕提示"请稍倾斜握持"。
验证（自然握姿：设备水平指前、屏幕朝上）：g0≈(0,0,1) → right=cross((-1,0,0),(0,0,1))=(0,1,0)=设备右侧 ✓；抬顶端 Δg≈(+0.5,0,-0.3)，fwd=(1,0,0)，p>0 升速 ✓。
