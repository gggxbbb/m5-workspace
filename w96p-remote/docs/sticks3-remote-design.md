...
- **数据采信两原则**（用户裁定，违反即设计错误）：
  1. **设备可换电池** → 一切设备端回报的电池记账信息不可信（`capacityMwh/rcapMwh/chgMwh/dchgMwh/chgTimeS/dchgTimeS` 都是固件按标称电池记的账）。界面只采信**电压与电流**；电量百分比只能做电压法估算，且必须标注"估算"。
  2. **设备无温度传感器** → `tempC`（FFD1）与 `coreTemp`（FFD4）均来自电源管理芯片，不是电池温度，**不采信、不呈现**。
...
- **电池行**：>30% 白、10-30% 橙、<10% 红（按电压估算 SOC 着色）；充电中电流值变绿
...
7. **电量估算（SOC%）**：只做电压法（VTC6 参考曲线查表，几十行无状态代码），显示必须带 `~` 和 `(est)`；SDK 的自学习算法（w96p-control/docs/battery-estimation.md，电压转移+曲线+持久化）可作为二期移植，但同样只能吃电压/电流两个输入。设备可换电池，换一次电池电压特性可能不同，估算值永远只是参考。
